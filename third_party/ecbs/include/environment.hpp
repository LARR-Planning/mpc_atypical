...
// This code is valid when grid_size > 2 * quad_size
//
    ...
    struct Conflict {
        ...
        friend std::ostream &operator<<(std::ostream &os, const Conflict &c) {
            switch (c.type) {
                ...
                case Edge:
                    return os << c.time << ": Edge(" << c.x1 << "," << c.y1 << "," << c.z1 << ","
                              << c.x2 << "," << c.y2 << "," << c.z2 << ","
                              << c.x3 << "," << c.y3 << "," << c.z3 << ")";
            }
            return os;
...
//                    }
                        ...
                        if ((state1a.equalExceptTime(state2b) || state1b.equalExceptTime(state2a)) &&
                            (!state1a.equalExceptTime(state1b) && !state2a.equalExceptTime(state2b)) &&
                ...
                for (size_t i = 0; i < solution.size(); ++i) {
                    ...
                    for (size_t j = i + 1; j < solution.size(); ++j) {
                        ...
                        if (m_quad_size[i] + m_quad_size[j] < m_grid_size * 0.5) {
                            ...
                            if (!state1a.equalExceptTime(state1b) &&
                                !state2a.equalExceptTime(state2b) &&
                                !isParallel(state1a, state1b, state2a, state2b)) {
                                if (state1b.equalExceptTime(state2a)) {
                                    result.time = t;
                                    result.agent1 = i;
                                    result.agent2 = j;
                                    result.type = Conflict::Edge;
        ...
        void createConstraintsFromConflict(
            ...
            if (conflict.type == Conflict::Vertex) {
                ...
                c2.edgeConstraints.emplace(EdgeConstraint(
                        conflict.time, conflict.x2, conflict.y2, conflict.z2, conflict.x3, conflict.y3, conflict.z3));
